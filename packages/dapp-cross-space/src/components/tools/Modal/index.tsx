...
const WaitFluentContent: React.FC = memo(() => {
    return (
        <div className="w-[400px] h-[140px] text-center bg-white rounded-lg">
            ...
            <p className="mt-[16px]">Confirm the transaction in your Fluent wallet...</p>
        </div>
    );
});
...
export const showWaitFluent = () => WaitFluentModal.show({ Content: <WaitFluentContent />, duration: 0, showMask: true, animationType: 'door' });

export const showTransactionSubmitted = (TxnHash: string) => {
    WaitFluentModal.hideAll();
    return TransactionSubmittedModal.show({ Content: <TransactionSubmittedContent TxnHash={TxnHash} />, duration: 0, showMask: true, animationType: 'door' });
};
