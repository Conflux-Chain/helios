...
class ConfluxManage {
    conflux = observable.computed(() => FluentManage.status.chainId ?  new Conflux(confluxNetworkConfig[FluentManage.status.chainId]) : undefined);
    CrossSpaceContract = observable.computed(() => this.conflux.value?.Contract(CrossSpaceContractConfig) as unknown as CrossSpaceContract);
}
