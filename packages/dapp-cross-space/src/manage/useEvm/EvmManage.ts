...
class EvmManage {
    networkConfig = observable.computed(() => FluentManage.status.chainId ? confluxNetworkConfig[FluentManage.status.chainId] : undefined);
    ...
    getBalance = async () => {
        ...
        try {
            ...
            } else if (balance?.error?.message === 'the method eth_getBalance does not exist/is not available')
                this.isSupportEvmSpace.value = false;
        } catch (err) {
...
export const trackEvmMappedAddressBalanceChangeOnce = Manage.trackEvmMappedAddressBalanceChangeOnce;
