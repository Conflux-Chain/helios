...
const Evm2Main: React.FC<{style: any}> = ({style}) => {
      ...
      try {
        ...
        const TxnHash = await sendTransaction({
          ...
          data: crossSpaceContract.withdrawFromMapped(
            convertCFX2Drip(+withdrawBalance - 0.001),
          ).data,
        })
        setInWithdraw(true)
  ...
  return (
    <a.div
      className="main-content backface-visible contain-content"
      style={style}
    >
      <p>
      ...
      <ul className="list-disc pl-6">
        ...
        <li>
          ...
          <span className="ml-1 text-red-500">following address</span>.
        </li>
        <li>
          This address can{' '}
          <span className="ml-1 text-red-500">{`only receive {CFX}.`}</span>
        </li>
      </ul>
...
const Withdraw: React.FC<{
  ...
  useEffect(() => {
    if (
      ...
      ((Number(preBalance.current) <= 0.01 &&
        Number(withdrawableBalance ?? 0) > 0.01) ||
  ...
  return (
    <>
      ...
      <p className="mt-4 mb-2 text-ellipsis overflow-hidden">
      ...
      <p className="">
        ...
        {inWithdraw
          ? ' in withdraw...'
          : typeof withdrawableBalance !== 'undefined'
          ? ` ${+withdrawableBalance > 0.01 ? withdrawableBalance : 0} CFX`
          : ''}
      </p>
      <button
        ...
        disabled={
          inWithdraw || !withdrawableBalance || +withdrawableBalance <= 0.01
        }
