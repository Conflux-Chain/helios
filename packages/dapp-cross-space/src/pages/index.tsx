...
const App: React.FC = () => {
    ...
    const [flipped, setFlipped] = useState(() => localStorage.getItem('filpped') === 'true');
    ...
    const handleClickFlipped = useCallback(() => setFlipped(pre => {
    ...
    return (
        <div className='w-fit m-auto pt-14'>
            ...
            {isConnected && 
                <>
                    <Main2Evm style={{ zIndex: flipped ? 0 : 1, opacity: opacity.to(o => 1 - o), transform }} />
                    <Evm2Main style={{ zIndex: flipped ? 1 : 0, opacity, transform, rotateY: '180deg' }} />
                    {!isSupportEvmSpace &&
                        <div className='fixed bottom-[24px] left-[50%] translate-x-[-46%] flex justify-center items-center px-[32px] py-[12px] bg-[#445159] text-white rounded overflow-hidden'>
                            It seems that current network doesn't support EVM Space, please check your network.
                        </div>
                    }
                </>
            }
