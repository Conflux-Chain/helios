...
type ConfluxChainId = ConfluxMainNetwork | ConfluxTestNetwork | ConfluxTest2Network;

type FluentEvents = 'chainChanged' | 'accountsChanged' | 'connect' | 'disconnect';

interface Window {
    conflux?: {
        isFluent: true;
        isConnected(): boolean;
        on(event: 'connect', cb: (param: { chainId: ConfluxChainId; networkId: number; }) => void): void;
        ...
        request(args: { method: 'wallet_getBalance'; params: [string]; }): Promise<string>;
        request(args: { method: 'cfx_sendTransaction'; params: [{ from: string; to: string; value?: string; data?: string;  }]; }): Promise<any>;
