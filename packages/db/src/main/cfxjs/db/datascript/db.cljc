(ns ^:no-doc ^:lean-ns cfxjs.db.datascript.db
  (:require
...
(declare hash-datom equiv-datom seq-datom nth-datom assoc-datom val-at-datom mem-only?)
...
#?(:cljs
   (defn pr-db [db w opts]
     ...
     (pr-sequential-writer w
                           (fn [d w opts]
                             ;; don't print/persist mem only attr
                             (when-not (mem-only? db (.-a d))
...
(defn #?@(:clj  [^Boolean mem-only?]
          :cljs [^boolean mem-only?]) [db attr]
  (is-attr? db attr :db/memOnly))
