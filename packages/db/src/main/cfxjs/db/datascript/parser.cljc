(ns ^:no-doc cfxjs.db.datascript.parser
  (:refer-clojure :exclude [distinct?])
  #?(:cljs (:require-macros [cfxjs.db.datascript.parser :refer [deftrecord]]))
  (:require
   [clojure.set :as set]
...
#?(:clj
   (defmacro deftrecord
     "Augment all cfxjs.db.datascript.parser/ records with default implementation of ITraversable"
     [tagname fields & rest]
     (let [f    (gensym "f")
           pred (gensym "pred")
           acc  (gensym "acc")]
       `(defrecord ~tagname ~fields
          ITraversable
          (~'-postwalk [this# ~f]
            (let [new# (new ~tagname ~@(map #(list 'cfxjs.db.datascript.parser/postwalk % f) fields))]
              (if-let [meta# (meta this#)]
                (with-meta new# meta#)
                new#)))
          (~'-collect [_# ~pred ~acc]
            ;; [x y z] -> (collect pred z (collect pred y (collect pred x acc)))
            ~(reduce #(list 'cfxjs.db.datascript.parser/collect pred %2 %1) acc fields))
          (~'-collect-vars [_# ~acc]
            ;; [x y z] -> (collect-vars-acc (collect-vars-acc (collect-vars-acc acc x) y) z)
            ~(reduce #(list 'cfxjs.db.datascript.parser/collect-vars-acc %1 %2) acc fields))
          ~@rest))))
