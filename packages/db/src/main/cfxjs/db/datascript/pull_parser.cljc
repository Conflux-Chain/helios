(ns ^:no-doc cfxjs.db.datascript.pull-parser
  (:require
   [cfxjs.db.datascript.built-ins :as built-ins]
   [cfxjs.db.datascript.db :as db #?(:cljs :refer-macros :clj :refer) [cond+ raise]]))
...
(defn parse-pattern ^PullPattern [db pattern]
  ...
  (loop [pattern pattern
    ...
    (cond+
     (empty? pattern)
     (let [attrs       (.-attrs ^PullPattern result)
           ...
           key-fn      (fn [^PullAttr attr]
                         (let [name (:name attr)]
                           (cond
                             ...
                             :eles (keyword name))))
           attrs       (if (and
                            (.-wildcard? ^PullPattern result)
                            (not (some db-id? (.-attrs ^PullPattern result))))
       ...
       (map->PullPattern
        {:attrs         attrs
         ...
         :last-attr     last-attr
         :reverse-attrs (list* (sort-by key-fn (.-reverse-attrs ^PullPattern result)))
         :wildcard?     (.-wildcard? ^PullPattern result)}))
