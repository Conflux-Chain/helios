...
/**
 * Normalize an address for case-insensitive signer/owner comparisons.
 */
 ...
 */
export const parseTypedData = value => {
...
/**
 * Identify supported ERC-2612/DAI Permit and Uniswap Permit2 payload shapes.
 */
export const detectPermitType = (options = {}) => {
  const {typedData, signerAddress} = options || {}
  ...
  const isPermit2Domain = domain?.name === 'Permit2'

  if (isPermit2Domain) {
    const isBatchAllowance = primaryType === 'PermitBatch'
    if (
      (primaryType === 'PermitSingle' || isBatchAllowance) &&
      hasFields(types, 'PermitDetails', PERMIT_DETAILS_FIELDS) &&
      hasFields(types, primaryType, [
        ['details', isBatchAllowance ? 'PermitDetails[]' : 'PermitDetails'],
        ['spender', 'address'],
        ['sigDeadline', 'uint256'],
      ]) &&
      hasKeys(message, ['details', 'spender', 'sigDeadline'])
    ) {
      return {
        type: 'permit2',
        mode: 'signature-allowance',
        ...(isBatchAllowance ? {isBatch: true} : {}),
        typedData,
      }
    }

    const isBatch = primaryType?.includes('Batch')
    const isWitness = primaryType?.includes('Witness')
    ...
    const validSignatureType =
      signatureTypes.has(primaryType) &&
      isEip712PrimaryTypeField(types, 'TokenPermissions', 'token', 'address') &&
      ...
      isEip712PrimaryTypeField(
        ...
        primaryType,
        'permitted',
        isBatch ? 'TokenPermissions[]' : 'TokenPermissions',
      ) &&
      isEip712PrimaryTypeField(types, primaryType, 'spender', 'address') &&
      isEip712PrimaryTypeField(types, primaryType, 'nonce', 'uint256') &&
      isEip712PrimaryTypeField(types, primaryType, 'deadline', 'uint256') &&
      hasKeys(message, [
        'permitted',
        'spender',
        'nonce',
  ...
  if (
    primaryType === 'Permit' &&
    hasFields(types, 'Permit', PERMIT_FIELDS) &&
    hasKeys(
      message,
      PERMIT_FIELDS.map(([name]) => name),
    )
  ) {
    return {
      type: 'permit',
      ...
      ownerMatchesSigner:
        !signerAddress ||
        normalizeAddress(message.owner) === normalizeAddress(signerAddress),
      typedData,
    }
  }
  ...
  if (
    primaryType === 'Permit' &&
    hasFields(types, 'Permit', DAI_PERMIT_FIELDS) &&
    hasKeys(
      message,
      DAI_PERMIT_FIELDS.map(([name]) => name),
    )
  ) {
    return {
      type: 'permit',
      ...
      ownerMatchesSigner:
        !signerAddress ||
        normalizeAddress(message.holder) === normalizeAddress(signerAddress),
      typedData,
    }
  }
...
/**
 * Detect a DAI-style Permit revocation payload (`allowed === false`).
 */
export const getIsRevokeDAIPermit = typedData => {
  const {message = {}, domain = {}, primaryType, types = {}} = typedData || {}
