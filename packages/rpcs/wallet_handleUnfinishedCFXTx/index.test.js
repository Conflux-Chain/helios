...
describe('wallet_handleUnfinishedCFXTx', () => {
  ...
  describe('resend', () => {
    test('Error in sendRawTx same nonce already inserted replace gas price, and tx has been speedup', async () => {
      const err = {data: 'same nonce already inserted replace gas price'}
      const inputs = mergeDeepObj(defaultInputs, {
        rpcs: {
          cfx_sendRawTransaction: vi.fn(() => Promise.reject(err)),
      ...
      expect(inputs.db.setTxPending).toHaveBeenCalledTimes(1)
      expect(inputs.db.setTxPending).toHaveBeenLastCalledWith({hash: 'txhash'})
      expect(inputs.db.queryTxWithSameNonce).toHaveBeenCalledTimes(1)
      expect(inputs.db.queryTxWithSameNonce).toHaveBeenLastCalledWith({
        hash: 'txhash',
      })
      ...
      expect(inputs.params.failedCb).toHaveBeenCalledTimes(0)
      expect(inputs.db.setTxFailed).toHaveBeenCalledTimes(0)
